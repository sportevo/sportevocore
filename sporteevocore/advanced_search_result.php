...
// trap sql / code injection attempt
    if ((mysql_real_escape_string($keywords) <> $keywords) || (strip_tags($keywords) <> $_GET['keywords'])){
    $error = true;
...
<!-- body_text //-->
<div class="pageHeading"><?php echo HEADING_TITLE_2; ?></div>
<div id="shipcontainer">

	

   
		<?php   if (CATEGORY_LIST_IN_SEARCH && CATEGORY_SEARCH_POSITION == 'Above') { 
			include (DIR_WS_MODULES . 'category_search_result.php');
			 } 
		 if (!(isset($_GET['scat_only']) && ($_GET['scat_only'] == '1'))) { ?>
...
// EOF Separate Pricing Per Customer
  ...
  for ($i=0, $n=sizeof($column_list); $i<$n; $i++) {
   ...
   // $select_str = "select distinct " . $select_column_list . " m.manufacturers_id, p.products_id, pd.products_name, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price, p.sold_in_bundle_only ";
...
<button class="cssButtonb"><?php echo '<a href="' . tep_href_link(FILENAME_ADVANCED_SEARCH, tep_get_all_get_params(array('sort', 'page')), 'NONSSL', true, false) . '">' . tep_image_button('button_back.gif', IMAGE_BUTTON_BACK) . '</a>'; ?></button>
