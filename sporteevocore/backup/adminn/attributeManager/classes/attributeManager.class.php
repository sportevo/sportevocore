	...
	function getAllTemplatesAndAttributes() {
		if(0 === count($this->arrAllTemplatesAndAttributes)) {
			...
			foreach($ref as $res) {
				...
				$this->arrAllTemplatesAndAttributes[$templateID]['options_quantity'][$res['option_values_id']] = $res['options_quantity'];			}
		}
					...
					foreach ($values['values'] as $optionValuesId => $allOptionValues){
						if (!AM_USE_SORT_ORDER) {
  							$data = array(
  								...
  								'options_quantity' => $values['values'][$optionValuesId]['options_quantity']
  							);
  						} else {
	  						$data = array(
  								'template_id' => $newTemplateId,
							  ...
							  if (!AM_USE_SORT_ORDER && !AM_USE_QT_PRO) {
  								$this->addAttributeToProduct(
  									array(
										...
										'options_quantity' => $allTemplatesAttributes[$templateId]['options_quantity'][$optionValuesId]  // JFJ - artikelnr per attribut
  									)
  								);
  							}
								if (AM_USE_SORT_ORDER && !AM_USE_QT_PRO) {
