  ...
  if (isset($_GET['action'])) {
    ...
    // Add a product to the virtual cart
      case 'add_product':
        ...
        // Get Product Info
        $product_query = tep_db_query("select p.products_model, p.products_cost, p.products_price, pd.products_name, p.products_tax_class_id from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on pd.products_id = p.products_id where p.products_id = '" . (int)$add_product_products_id . "' and pd.language_id = '" . $languages_id . "'");
		...
		// Update inventory Quantity
			...
			options_quantity = options_quantity - " . $_POST['add_product_quantity'] . " 
			WHERE products_id = '" . $_POST['add_product_products_id'] . "' AND options_id = '" . $option_id . "' AND options_values_id = '" . $option_value_id . "'");
...
// Generate product list based on chosen category or search keywords
  ...
  if (isset($_POST['search'])) {
    ...
    $products_query = tep_db_query("select p.products_id, p.products_cost, p.products_price, p.products_model, pd.products_name from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on (p.products_id = pd.products_id) where pd.language_id = '" . $languages_id . "' and (" . $product_search . ") order by pd.products_name");
  ...
  if (!isset($_POST['search'])) {
    ...
    $products_query = tep_db_query("select distinct p.products_id, p.products_cost, p.products_price, p.products_model, pd.products_name from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_CATEGORIES . " c, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c " . $product_search . " order by pd.products_name");
    $not_found = ((tep_db_num_rows($products_query)) ? false : true);
  }
