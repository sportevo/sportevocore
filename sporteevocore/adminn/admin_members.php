...
//STart Code added to add manufacture and vendor- arun	
if($_POST['admin_groups_id']=='5'){
	  
	    $manufacturers_name = tep_db_prepare_input($_POST['admin_firstname']).' '.tep_db_prepare_input($_POST['admin_lastname']);
        $sql_data_array_manf = array('manufacturers_id' => $admin_id,
		                       'manufacturers_name' => $manufacturers_name);
        $insert_sql_data_manf = array('date_added' => 'now()');
        $sql_data_array_manf = array_merge($sql_data_array_manf, $insert_sql_data_manf);
        tep_db_perform(TABLE_MANUFACTURERS, $sql_data_array_manf);
		$manufacturers_id = tep_db_insert_id();
		
		$insert_sql_data_maf = array('manufacturers_id' => $admin_id,
                                     'languages_id' => '1', 'manufacturers_htc_title_tag' => $manufacturers_name);
      
		tep_db_perform(TABLE_MANUFACTURERS_INFO, $insert_sql_data_maf);
		  	
		 $sql_data_array_vend = array (
		   'vendors_id' => $admin_id,
          'vendors_name' => $manufacturers_name,
          'vendors_email' => tep_db_prepare_input($_POST['admin_email_address']),
          'vendors_send_email' => tep_db_prepare_input($_POST['admin_email_address']),
        );
		 tep_db_perform(TABLE_VENDORS, $sql_data_array_vend);
		 
	}	 
//End Code added to add manufacture and vendor- arun	
...
// EOF: KategorienAdmin / OLISWISS                                 
	...
	if($_POST['admin_groups_id']=='5'){	  
	    ...
	    $manufacturers_name = tep_db_prepare_input($_POST['admin_firstname']).' '.tep_db_prepare_input($_POST['admin_lastname']);
        $sql_data_array_manf = array('manufacturers_name' => $manufacturers_name);
        $insert_sql_data_manf = array('date_added' => 'now()');
        $sql_data_array_manf = array_merge($sql_data_array_manf, $insert_sql_data_manf);
        tep_db_perform(TABLE_MANUFACTURERS, $sql_data_array_manf, 'update', 'manufacturers_id = \'' . $admin_id . '\'');
		$manufacturers_id = tep_db_insert_id();
		
		$insert_sql_data_maf = array('manufacturers_id' => $admin_id,
                                     'languages_id' => '1', 'manufacturers_htc_title_tag' => $manufacturers_name);
      
		tep_db_perform(TABLE_MANUFACTURERS_INFO, $insert_sql_data_maf, 'update', 'manufacturers_id = \'' . $admin_id . '\'');
		 ...
		 tep_db_perform(TABLE_VENDORS, $sql_data_array_vend, 'update', 'vendors_id = \'' . $admin_id . '\'');
...
//End Code added to add manufacture and vendor- arun	
      ...
      case 'member_delete':
        ...
        tep_db_query("delete from " . TABLE_ADMIN . " where admin_id = '" . $admin_id . "'");
		 tep_db_query("delete from " . TABLE_MANUFACTURERS . " where manufacturers_id = '" . $admin_id . "'");
		  tep_db_query("delete from " . TABLE_MANUFACTURERS_INFO . " where manufacturers_id = '" . $admin_id . "'");
		  tep_db_query("delete from " . TABLE_VENDORS . " where vendors_id = '" . $admin_id . "'");
    ...
    case 'edit_member':
      ...
      if (strstr($str_rights,"PDELETE")) $pdelete_checked = true;false;
        ...
        while ($n < tep_db_num_rows($top_categories_query)) {
          ...
          $top_categories_name_query = tep_db_query("select categories_name from " . TABLE_CATEGORIES_DESCRIPTION . " where language_id=". (int)$languages_id . " and categories_id=" . $top_categories['categories_id']);
...
// EOF: KategorienAdmin / OLISWISS
	...
	if ($str_cat_no == "") {
	  ...
	  while ($n < count($my_categories_array)) {
	    $admin_category_query = tep_db_query("select categories_name from " . TABLE_CATEGORIES_DESCRIPTION . " where language_id=". (int)$languages_id . " and categories_id=" . $my_categories_array[$n]) ;
