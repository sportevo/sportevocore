...
if ($_GET['action'] == 'export') {
 ...
 $orders = "";
$orders_query = tep_db_query("SELECT * from " . TABLE_ORDERS . " ORDER by orders_status, orders_id desc");
while($orders_rows = tep_db_fetch_array($orders_query))

 
 $csv_output .= "\n";
  $csv_query = tep_db_query("SELECT * FROM " . TABLE_ORDERS_PRODUCTS . " WHERE orders_id = '" . $orders_rows['orders_id'] . "'  ");


while($products_rows = tep_db_fetch_array($csv_query))
{
	

$attributes = "";
$attributes_query = tep_db_query("SELECT * FROM " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " WHERE orders_products_id = ' " .  $products_rows['orders_products_id'] . "'" );
$attributes_rows = tep_db_fetch_array($attributes_query);
...
$ordStatus_rows = tep_db_fetch_array($ordStatus_query);
    ...
    {
       $csv_output .= utf8_decode($products_rows['products_name']) . ";" . $products_rows['products_options_values_name'] . ";" . ";" . $products_rows['products_quantity'] . ";" . str_replace(".",",",$csv['products_cost']) . ";" . str_replace(".",",",$csv['products_cost']*$csv['options_quantity']) . ";" . str_replace(".",",",$csv['products_price']) . ";" . str_replace(".",",",$csv['products_price']*$csv['options_quantity']) . ";" . str_replace(".",",",($csv['products_price']-$csv['products_cost'])/$csv['products_price']*'100') . ";" . str_replace(".",",",($csv['products_price']*$csv['options_quantity'])-($csv['products_cost']*$csv['options_quantity'])) . ";"  . $csv['products_status'] . ";" . $csv['products_id'] . ";" . $csv['products_model'] . ";" . $csv['manufacturers_name'] . ";" . $csv['products_weight'] . "\n";
      }
}
//salva con nome - ONTC
   $saveas = 'Stats_report';
...
?>
		  ...
		  <tr>
              ...
              <tr>
                <td colspan="4" align="left"><?php echo tep_draw_form('stockprice_report', 'inventory_report2.php', 'action=export', 'post'); ?><?php echo tep_image_submit('button_csv_export.gif', IMAGE_EXPORT); ?></form></td>
              </tr>
          <tr>
