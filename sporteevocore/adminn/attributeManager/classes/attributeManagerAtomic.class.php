...
// JFJ - artikelnr per attribut BOF
  if(isset($get['options_quantity']) === true  ) {
    $this->getAndPrepare('options_quantity', $get, $getArray['options_quantity']);
  }
// JFJ - artikelnr per attribut EOF
...
// JFJ - artikelnr per attribut
		...
		foreach($this->arrSessionVar as $id => $res) {
			if(($res['option_id'] == $getArray['option_id']) && ($res['option_value_id'] == $getArray['option_value_id'])) {
				...
				$this->arrSessionVar[$id]['options_quantity'] = $getArray['options_quantity'];   // JFJ - artikelnr per attribut
				if (AM_USE_SORT_ORDER) {
	...
	function getAllProductOptionsAndValues($reset = false) {
		...
		for($z1=0;$z1<count($this->arrSessionVar);$z1++){
			...
			foreach($this->arrSessionVar as $id => $res) {
				...
				$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['options_quantity'] = $res['options_quantity'];  // JFJ - artikelnr per attribut
				if (AM_USE_SORT_ORDER) {
	...
	function sortProductAttributes(){
		...
		while( list($key, $currentOption) = each($arrAllProductOptionsAndValues) ){
			while( list($currentOptionValuekey, $currentOptionValues) = each($currentOption['values']) ){
					...
					$sortedArray[$sortorder] =  array( 'optionValue-id' => $currentOptionValuekey,
														...
														'optionValue-options_quantity' => $currentOptionValues['options_quantity'],  // JFJ - artikelnr per attribut
		...
		while( list($key, $currentOption) = each($sortedArray) ){
			...
			$optionValues[$currentOption['optionValue-id']] = array(
																	...
																	'options_quantity' => $currentOption['optionValue-options_quantity'],  // JFJ - artikelnr per attribut
																);
