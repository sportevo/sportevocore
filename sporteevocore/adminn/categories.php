 ...
 if (USE_PRODUCTS_COUNT_CACHE == 'true') {
	...
	// DDB - 040606 - add copy of category - BOF
      case 'copy_category_confirm':
		...
		{
          $old_categories_id = tep_db_prepare_input($_POST['categories_id']);
          $parent_id = tep_db_prepare_input($_POST['copy_to_category_id']);
...
//MVS end
                                ...
                                'products_master_status' => tep_db_prepare_input($_POST['products_master_status']),
								  ...
								  'manufacturers_id' => tep_db_prepare_input($_POST['vendors_id']),// added by arun
...
//MVS end
                                    ...
                                    'products_master_status' => tep_db_prepare_input($_POST['products_master_status']),
                                    'products_tax_class_id' => tep_db_prepare_input($_POST['products_tax_class_id']),
                                    'manufacturers_id' => (int)tep_db_prepare_input($_POST['vendors_id']));
...
<!-- Master Products //-->
                    <td align="right" class="smallText">
                    <?php if (sizeof($cPath_array) > 0) echo tep_image_button(IMAGE_BACK, 'triangle-1-w', tep_href_link(FILENAME_CATEGORIES, $cPath_back . 'cID=' . $current_category_id)); if (!isset($_GET['search'])) echo tep_image_button(IMAGE_NEW_CATEGORY, 'plus', tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&action=new_category')) . tep_image_button(IMAGE_NEW_MASTER, 'plus', tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&action=new_master')) . tep_image_button(IMAGE_NEW_PRODUCT, 'plus', tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&action=new_product')); ?>&nbsp;</td>
<!-- Master Products EOF //-->
