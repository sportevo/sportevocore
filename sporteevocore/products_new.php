...
<?php
  ...
  $products_new_query_raw = "select p.products_id, pd.products_name, p.products_image, m.manufacturers_name, p. sold_in_bundle_only from " . TABLE_PRODUCTS . " p left join " . TABLE_MANUFACTURERS . " m on (p.manufacturers_id = m.manufacturers_id), " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by p.products_date_added DESC, pd.products_name";
...
// replace specials_new_products_price with those those for the customers_group_id
	...
	if (tep_not_null($products_new[$x]['specials_new_products_price'])) {
	        $products_price = '<s>' . $currencies->display_price($products_new[$x]['products_price'] > 0, tep_get_tax_rate($products_new[$x]['products_tax_class_id'])) . '</s> <span class="productSpecialPrice">' . $currencies->display_price($products_new[$x]['specials_new_products_price'], tep_get_tax_rate($products_new[$x]['products_tax_class_id'])) . '</span>';
	} else {
		$products_price = $currencies->display_price($products_new[$x]['products_price'] > 0, tep_get_tax_rate($products_new[$x]['products_tax_class_id']));
	}
?>
          <tr>
            ...
            <td valign="top" class="main"><?php echo '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_new[$x]['products_id']) . '"><b><u>' . $products_new[$x]['products_name'] . '</u></b></a><br>' . TEXT_DATE_ADDED . ' ' . tep_date_long($products_new[$x]['products_date_added']) . '<br>' . TEXT_MANUFACTURER . ' ' . $products_new[$x]['manufacturers_name'] . '<br><br>' ?></td>
            
            
         
            
            <td align="right" valign="middle" class="main">
            <?php if ($products_new['sold_in_bundle_only'] == "yes") {
